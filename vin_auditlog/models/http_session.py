...
class AuditlogtHTTPSession(models.Model):
    _name = "vin_auditlog.http.session"
    ...
    http_request_ids = fields.One2many(
        "vin_auditlog.http.request", "http_session_id", string="HTTP Requests"
    )
    ...
    def current_http_session(self):
        ...
        if httpsession:
            ...
            httpsession["vin_vin_auditlog_http_session_id"] = self.create(vals).id
            return httpsession.get("vin_vin_auditlog_http_session_id")
        return False
