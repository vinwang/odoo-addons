...
class AuditlogHTTPRequest(models.Model):
    _name = "vin_auditlog.http.request"
    ...
    log_ids = fields.One2many("vin_auditlog.log", "http_request_id", string="Logs")
    ...
    def current_http_request(self):
        ...
        if httprequest:
            if hasattr(httprequest, "vin_vin_auditlog_http_request_id"):
                # Verify existence. Could have been rolled back after a
                # concurrency error
                self.env.cr.execute(
                    "SELECT id FROM %s WHERE id = %s",
                    (AsIs(self._table), httprequest.vin_vin_auditlog_http_request_id),
                )
                if self.env.cr.fetchone():
                    return httprequest.vin_vin_auditlog_http_request_id
            vals = {
            ...
            httprequest.vin_vin_auditlog_http_request_id = self.create(vals).id
            return httprequest.vin_vin_auditlog_http_request_id
        return False
