...
class AuditlogCommon:
    ...
    def test_LogCreation(self):
        ...
        self.assertEqual(
            self.env["vin_auditlog.log"].search_count(
                [
    ...
    def test_LogCreation2(self):
        ...
        vin_vin_auditlog_log = self.env["vin_auditlog.log"]
        testgroup2 = self.env["res.groups"].create({"name": "testgroup2"})
        self.assertTrue(
            vin_vin_auditlog_log.search(
                [
                    ("model_id", "=", self.groups_model_id),
                    ("method", "=", "create"),
    ...
    def test_LogCreation4(self):
        ...
        logs = vin_vin_auditlog_log.search(
            [
                ("model_id", "=", self.groups_model_id),
                ("method", "=", "create"),
    ...
    def test_LogCreation5(self):
        ...
        testgroup5.unlink()
        log_record = vin_vin_auditlog_log.search(
            [
                ("model_id", "=", self.groups_model_id),
                ("method", "=", "unlink"),
                ("res_id", "=", testgroup5.id),
    ...
    def test_LogUpdate(self):
        ...
        log1 = self.env["vin_auditlog.log"].search(
            [
                ("model_id", "=", self.groups_model_id),
                ("method", "=", "write"),
                ("res_id", "=", group.id),
        ...
        log2 = self.env["vin_auditlog.log"].search(
            [
                ("model_id", "=", self.groups_model_id),
                ("method", "=", "write"),
                ("res_id", "=", group.id),
        ...
        log3 = self.env["vin_auditlog.log"].search(
            [
                ("model_id", "=", self.groups_model_id),
                ("method", "=", "write"),
                ("res_id", "=", group.id),
...
class TestAuditlogExportData(AuditLogRuleCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.groups_model_id = cls.env.ref("base.model_res_groups").id
        cls.groups_rule = cls.env["vin_auditlog.rule"].create(
            {
                "name": "testrule for groups",
                "model_id": cls.groups_model_id,
    ...
    def test_LogExport(self):
        ...
        created_log = vin_vin_auditlog_log.search(
            [
                ("model_id", "=", self.groups_model_id),
                ("method", "=", "export_data"),
            ]
...
class TestFieldRemoval(AuditLogRuleCommon):
    ...
    def setUpClass(cls):
        ...
        # Setup auditlog rule
        cls.vin_vin_auditlog_rule = cls.create_rule(
            {
                "name": "test.model",
                "model_id": cls.test_model.id,
                "log_type": "fast",
    ...
    def assert_values(self):
        ...
        self.vin_vin_auditlog_rule.invalidate_recordset()
        self.assertEqual(self.vin_vin_auditlog_rule.model_name, "x_test_model")
        self.assertEqual(self.vin_vin_auditlog_rule.model_model, "x_test.model")
    ...
    def test_01_field_and_model_removal(self):
        ...
        # Assert rule values
        self.assertFalse(self.vin_vin_auditlog_rule.model_id)
...
class AuditLogRuleTestForUserFields(AuditLogRuleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.vin_vin_auditlog_rule.fields_to_exclude_ids = [[4, cls.fields_to_exclude_ids]]

        # Updating users_to_exclude_ids
        cls.vin_vin_auditlog_rule.users_to_exclude_ids = [[4, cls.users_to_exclude_ids]]

        # Subscribe vin_auditlog.rule
        cls.vin_vin_auditlog_rule.subscribe()

        cls.vin_vin_auditlog_log = cls.env["vin_auditlog.log"]
    ...
    def test_01_AuditlogFull_field_exclude_create_log(self):
        ...
        create_log_record = self.vin_vin_auditlog_log.search(
            [
                ("model_id", "=", self.vin_vin_auditlog_rule.model_id.id),
                ("method", "=", "create"),
    ...
    def test_02_AuditlogFull_field_exclude_write_log(self):
        ...
        # Checking log is created for testpartner1
        write_log_record = self.vin_vin_auditlog_log.search(
            [
                ("model_id", "=", self.vin_vin_auditlog_rule.model_id.id),
                ("method", "=", "write"),
                ("res_id", "=", self.testpartner1.id),
    ...
    def test_03_AuditlogFull_user_exclude_write_log(self):
        ...
        with self.assertRaises(ValueError):
            self.vin_vin_auditlog_log.search(
                [
                    ("model_id", "=", self.vin_vin_auditlog_rule.model_id.id),
                    ("method", "=", "write"),
                    ("res_id", "=", partner.id),
    ...
    def test_06_AuditlogFull_unlink_log(self):
        ...
        delete_log_record = self.vin_vin_auditlog_log.search(
            [
                ("model_id", "=", self.vin_vin_auditlog_rule.model_id.id),
                ("method", "=", "unlink"),
                ("res_id", "=", self.testpartner1.id),
...
class AuditLogRuleTestForUserModel(AuditLogRuleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.group = cls.env.ref("vin_auditlog.group_vin_vin_auditlog_manager")
